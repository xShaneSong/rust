...
impl<'a> Parser<'a> {
    pub fn new(expr: &'a str) -> Result<Self, ParseError> {
        ...
        let cur_token = match lexer.next() {
            Some(token) => token,
            None => return Err(ParseError::InvalidOperator("Invalid character in expression".into())),
        };
        Ok(Parser {
            tokenizer: lexer,
    ...
    fn check_paren(&mut self, expected: Token) -> Result<(), ParseError> {
        if self.current_token != expected {
            return Err(ParseError::InvalidOperator(format!("Expected {:?} but got {:?}", expected, self.current_token)));
        }
        self.get_next_token()?;
        Ok(())
    }
...
impl fmt::Display for ParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match &self {
            ParseError::UnableToParse(ref err) => write!(f, "Unable to parse: {}", err),
            ParseError::InvalidOperator(ref err) => write!(f, "Invalid operator: {}", err),
        }
    }
}
